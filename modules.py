...
class FilterLayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        x = torch.fft.rfft2(input_tensor, dim=1, norm='ortho') # 将输入的数据进行傅里叶变换
        weight = torch.view_as_complex(self.complex_weight) #可学习的过滤器
        x = x * weight #进行过滤操作
        sequence_emb_fft = torch.fft.irfft2(x, n=seq_len, dim=1, norm='ortho') #对过滤后的频谱重新从频域转换为一个序列
        hidden_states = self.out_dropout(sequence_emb_fft)
        hidden_states = self.LayerNorm(hidden_states + input_tensor) # 对应公式7
